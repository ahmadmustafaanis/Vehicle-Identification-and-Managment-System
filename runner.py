...
class Runner:
    def __init__(self, input_path, number) -> None:
        self.db = db
        self.db.create_all()
    ...
    def insert_car(self, best_number_plate, cropped_car):
        ...
        if is_suspicious:
            ...
            self.sms_client.messages.create(
                messaging_service_sid="XXXXXXXXXXXXXXXXXXXXXXXXX",
                body=f'Suspicious Car Alert: Car Number "{best_number_plate}" inside premsis',
                to=self.number,
            )

            self.sms_client.messages.create(
                from_="whatsapp:+XXXXXXXXX6",
                body=f'Suspicious Car Alert: Car Number "{best_number_plate}" inside premsis',
            ...
            emit("suspicious_car", {"license_plate": best_number_plate})
        else:
            ...
            try:
                ...
                self.sms_client.messages.create(
                    messaging_service_sid="MGadc9bf1c0029afdbdd24020a78c9a73b",
                    body=f'Unknown Car Alert: Car Number "{best_number_plate}" inside premsis',
                    to=self.number,
                )
